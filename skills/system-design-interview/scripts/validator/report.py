...
from .depth_eval import DepthReport
...
def generate_report(
    ...
    lines.append(f"| Technical depth eval | {depth_pass} | {depth_total} | {'PASS' if depth.passed else 'FAIL'} |")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    # Technical Depth Eval
    lines.append("## Technical Depth Eval")
    lines.append("")
    lines.append("| Check | Count | Target | Status | Evidence |")
    lines.append("|-------|-------|--------|--------|----------|")
    for c in depth.checks:
        evidence = c.evidence.replace("|", "\\|")[:100]
        lines.append(f"| {c.name} | {c.count} | >= {c.target} | **{c.status}** | {evidence} |")
    lines.append("")
    lines.append(f"**Depth score**: {depth.score:.0f}%")
    lines.append("")

    for c in depth.checks:
        if c.examples and not c.passed:
            lines.append(f"**{c.name}** (needs improvement):")
    ...
    failed_depth = [c for c in depth.checks if not c.passed]

    if not failed_gates and not failed_consistency and not failed_quality and not failed_depth:
        ...
        if failed_quality:
            ...
            lines.append("")
        if failed_depth:
            lines.append("### Technical Depth Gaps")
            for c in failed_depth:
                lines.append(f"- **{c.name}**: found {c.count}, need >= {c.target}. {c.evidence}")
            lines.append("")
...
def write_report(
    ...
    content = generate_report(folder, structural, consistency, quality, depth)
