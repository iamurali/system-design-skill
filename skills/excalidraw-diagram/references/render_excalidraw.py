...
Usage:
    ...
    uv run python render_excalidraw.py <path> --output out.png --scale 2 --width 1920
...
def compute_bounding_box(elements: list[dict]) -> tuple[float, float, float, float]:
    """Compute bounding box (min_x, min_y, max_x, max_y) across all visible elements."""
    min_x = float("inf")
    min_y = float("inf")
    max_x = float("-inf")
    max_y = float("-inf")
...
def render(
    ...
    try:
        ...
        print(
            "Run: cd .cursor/skills/excalidraw-diagram/references && uv sync && uv run playwright install chromium",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(
                    "Run: cd .cursor/skills/excalidraw-diagram/references && uv run playwright install chromium",
                    file=sys.stderr,
                )
                sys.exit(1)
            raise
...
def main() -> None:
    ...
    parser.add_argument(
        "--width", "-w", type=int, default=1920, help="Max viewport width (default: 1920)"
    )
